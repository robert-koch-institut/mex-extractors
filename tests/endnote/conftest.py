...
def endnote_records() -> list[EndnoteRecord]:
    return [EndnoteRecord.model_validate({
        "abstract": "abstract.",
...
@pytest.fixture
def endnote_bibliographic_resource_mapping(settings: Settings) -> BibliographicResourceMapping:
    """Return confluence-vvt activity mapping from assets."""
...
def extracted_endnote_persons_by_person_string(
    extracted_primary_sources: dict[str,ExtractedPrimarySource],) -> dict[str,ExtractedPerson]:
    return {
